...
export const getRanking = async (options?: RequestInit): Promise<GetRanking200> => {
  const res = await fetch(getGetRankingUrl(), { ...options, method: 'GET' })

  if (!res.ok) {
    console.error('Erro ao buscar ranking:', res.status, await res.text())
    return { ranking: [] } // Retorna um array vazio para evitar erro no .map()
  }

  try {
    const body = await res.json()
    return { ranking: Array.isArray(body.ranking) ? body.ranking : [] } // Garante que ranking sempre seja um array
  } catch (error) {
    console.error('Erro ao processar JSON:', error)
    return { ranking: [] } // Retorna um array vazio em caso de erro
  }
}
